...
对于n维空间中的m维流形$\mathcal{M} \subset \mathbb{R}^n$，记它的向量场为$\mathbf{v}: \mathcal{M} \rightarrow \mathbb{R}^n$。我们对欧式空间中散度的定义进行推广，在流形上的散度可以定义为流形到实数域的映射：
...
实际上这样定义的$\bar{\phi}$在局部是一个**单射(injective)**，在足够小的邻域内甚至是**双射(bijective)**。因此$\mathbf{p}$点附近的向量都可以通过$\bar{\phi}^{-1}$映射回切平面上。
...
这样就可以把$\mathbb{R}^n$上散度的定义推广到流形上：

$$
\begin{aligned}
\nabla \cdot \bar{\mathbf{v}} &= \sum_{k=1}^m \mathbf{e}_k \cdot d \bar{\mathbf{v}} (\mathbf{e}_k) + \sum_{m+1}^n \bar{\mathbf{n}}_k \cdot d \bar{\mathbf{v}} (\bar{\mathbf{n}}_k) \\
