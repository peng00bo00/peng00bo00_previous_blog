...
需要说明的是这里只是借用了偏导数和微分的符号，实际上并没有任何的求导运算，只需要把它们想成是基就好。有了这些基础就可以进行计算了。比如说对于$\mathbb{R}^2$上的向量场$X = (1-x) \frac{\partial}{\partial x} + (1-y) \frac{\partial}{\partial y}$和differen 1-form $\alpha = x dx$，把$\alpha$作用到$X$上可以得到标量函数$\alpha (X)$：
...
下面介绍本课程最重要的概念之一"外微分(exterior derivative)",这里直接给出外微分的定义：

设$\Omega^k$是differential k-form构成的空间，外微分算子$d: \Omega^k \rightarrow \Omega^{k+1}$是一个**线性**映射，且满足如下性质:
- 微分性(differential)：对于0-form，有$d \phi(X) = D_X \phi$
- 乘法法则(product rule)：对于k-form $\alpha$，$d (\alpha \wedge \beta) = d \alpha \wedge \beta + (-1)^k \alpha \wedge d \beta$
- 恒等式(exactness)：$d \circ d = 0$
{:.info}

#### Differential

微分性说明对于任意的0-form $\phi$，它的(外)微分为:
...
可以发现求外微分类似于求梯度。但是外微分和梯度是不完全一致的：梯度是依赖于内积定义的，而外微分与内积的定义无关。换句话说，梯度需要通过内积来转换成外微分：
