...
对于$\mathbb{R}^n$上的k-form $\omega$，我们定义对应的离散形式为单纯复形积分的结果，即
...
这样的从连续到离散的映射称为discretization map或de Rham map。简单来说，离散化就是对网格的顶点、边和面元指定不同类型的对象：
...
最后我们来介绍一下hodge star($\star$)的离散形式。定义离散hodge star为k-form原始空间到其对偶空间的映射$\star: \Omega_k \rightarrow \Omega_{n-k}^\star$

$$
\widehat{\star \alpha}(\sigma^\star) = \frac{\lvert \sigma^\star \rvert}{\lvert \sigma \rvert} \hat{\alpha}(\sigma)
$$

其中$\sigma^\star$表示$\sigma$对应的dual cell。也就是说hodge star仅仅是对k-form进行了一定的缩放，因此其矩阵形式为一对角矩阵：
