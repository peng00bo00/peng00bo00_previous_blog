...
此外，我们还可以通过随机游走来认识Laplace算子。对于初始状态为$\phi$的随机游走，可以建立热传导方程：
...
$$
\Delta \phi = \lim_{t \to 0} \frac{1}{t} (u(t) - u(0)) = \frac{d}{dt} \Big\vert_{t=0} \mathbb{E}[\phi (X_t)]
$$

其中$k_t(x, y)$称为**热核(heat kernel)**，表示从具有单位温度的热源$x$经时间$t$传播到$y$处时的温度；$X_t$表示随机游走的粒子在$t$时刻的位置；而Laplace算子$\Delta u$则表达了粒子在任意时刻扩散的程度。


最后来介绍Laplace算子与**Dirichlet能量(Dirichlet energy)**的关系。
