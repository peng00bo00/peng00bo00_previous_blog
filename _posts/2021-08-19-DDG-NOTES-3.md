...
由于离散Laplace算子和平均曲率向量具有相同的形式，实际上平均曲率流也可以看做是求解热传导方程。取$u$为顶点坐标的函数，带入热传导方程并进行离散得到：
...
上式为热传导方程的**显示迭代(explicit update)**。实际工程中更常用的是**隐式迭代(implicit update)**，此时将迭代式修改为：
...
除此之外，泊松方程也可以看做是最小化Dirichlet能量。假设已知向量场$X$，我们希望寻找一个势场$u$使得$u$可以表示$X$，因此构造Dirichlet能量：
