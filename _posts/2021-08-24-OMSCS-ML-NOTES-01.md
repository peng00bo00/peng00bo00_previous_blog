...
最后我们来讨论一下使用决策树时一些其他的常见问题，首先是对连续型特征的处理。在前面的讨论中我们都假定特征是离散型随机变量，但在实际应用中有大量的连续型特征(年龄、身高、体重等)。决策树算法是不能直接应用在连续型随机变量上的，因此在使用决策树算法前首先需要对连续型随机变量进行离散化。常见的离散化方法是使用一些阈值对变量进行划分，从而将连续型随机变量转换为离散型随机变量；此外也可以结合特定任务相关的知识来进行离散化。
