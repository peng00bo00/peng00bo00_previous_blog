...
决策树也可以用来求解回归问题。我们把决策树看做是对样本空间进行划分，对于回归问题在每个子空间(叶节点)内可以利用样本的均值来作为模型的输出，这个过程相当于在子空间内最小化平方误差。因此使用决策树求解回归问题的核心是如何划分样本空间，也就是如何选择和划分样本的特征。在CART算法中使用了启发式的算法来进行处理，每次构建决策树时对当前数据集中每个特征以及每个取值进行遍历构造出两个相应的子空间分别计算数据集在子空间内的平方误差，然后选择平方误差最小的特征和取值作为当前数据集的切分点继续向下递归。
