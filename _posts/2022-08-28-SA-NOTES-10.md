...
同时，**Bourgain定理(Bourgain's theorem)**指出我们可以把有限度量空间近似等距地映射到一个非常低维的欧式空间中。
...
利用Gram矩阵就可以把样本的重建问题分解为2步：首先利用$P$矩阵恢复Gram矩阵，然后再利用Gram矩阵来重建样本坐标。
...
实际上上面推导的算法称为**classical multidimensional scaling(classical MDS)**，它在机器学习以及数据可视化中都有重要的应用。
...
其中$\mathbf{p}_i$是$\mathbf{P}$矩阵的第$i$列，而$\mathbf{g}=\text{diag}(\bar{X}^T \bar{X})$。上式的意义在于当我们得到新的数据时无需重新计算特征值分解，而是可以利用已知的信息来计算新样本点的嵌入。
...
&= \text{tr} \big( \mathbf{G} (2n \ \mathbf{I} - \mathbf{1} \mathbf{1}^T) \big) \\
...
显然原始的优化目标等于$\tau (\mathbf{X}, \mathbf{X})$，而且可以证明任意的矩阵$\mathbf{Z}$都是原始目标的一个上界。这里简单证明一下，首先根据Cauchy-Schwarz不等式有：
...
因此SMACOF算法的本质是优化原始目标的一个凸的上界来最小化目标函数。尽管这种方法理论上并不能保证收敛到全局最优值，但在大量的实践表明它可以收敛到一个足够好的解上。
