...
接下来我们假设空间中存在向量场$\mathbf{v}(t): (a, b) \rightarrow \mathbb{R}^2$满足$\mathbf{v}(a)=\mathbf{v}(b)=\mathbf{0}$。我们的目标是计算$L$在$\mathbf{v}$扰动下的Gateaux导数$d L_\gamma [\mathbf{v}]$，为此可以定义一组曲线：
