...
但使用旋转矩阵来进行表示有一些缺点，比如说旋转矩阵过于冗余了，它需要9个参数才能表示旋转但实际上我们只需要3个参数就可以了。同时，旋转矩阵非常不直观，对它进行求导也很不方便。
...
- 加减法：$\mathbf{q}_1 \plusmn \mathbf{q}_2 = [s_1 \plusmn s_2, \mathbf{v}_1 \plusmn \mathbf{v}_2]$
...
假设物体绕$\mathbf{v}$轴旋转了$\theta$度，则旋转的过程可以使用一个单位四元数来表示：
...
在计算刚体的旋转时我们同样需要在时间上进行积分：

$$
\bm{\omega}^{[1]} = \bm{\omega}^{[0]} + \Delta t (\mathbf{I}^{[0]})^{-1} \bm{\tau}^{[0]}
$$

$$
\mathbf{q}^{[1]} = \mathbf{q}^{[0]} + [0, \ \frac{\Delta t}{2} \bm{\omega}^{[1]}] \times \mathbf{q}^{[0]}
$$
...
类似于质量与加速度，在计算刚体旋转时我们有**力矩(torque)**和**转动惯量(inertia)**的概念。但需要注意的是在每一时刻我们都需要对刚体当前的转动惯量进行更新：
...
<div align=center>
<img src="https://i.imgur.com/QovpyAK.png" width="70%">
</div>
