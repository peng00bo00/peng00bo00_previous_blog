...
其中$[ \ ]$为**李括号(Lie bracket)**，$[X, Y] = \sum_{i, j} (X^j \frac{\partial}{\partial u_j} Y^i -Y^j \frac{\partial}{\partial u_j} X^i) \frac{\partial}{\partial u^i}$。同时可以证明协变导数由曲面的黎曼度量$g$所唯一确定。
