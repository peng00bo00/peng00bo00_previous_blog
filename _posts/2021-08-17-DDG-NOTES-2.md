...
对于凸多面体，对每个顶点的angle defect求和相当于计算单位球的表面积($4 \pi$)，也就是说angle defect的和（积分）是某种拓扑不变量。实际上angle defect就是离散曲面的Gauss曲率，而且同样满足Gauss-Bonnet定理：
...
在几何处理中更为常用的曲率是**曲率向量**，即大小等于曲率方向与法向相同的向量。前面已经推导了**面积法向(area normal)**为$\frac{1}{2} df \wedge df = N dA$，类似地可以推导**平均曲率法向(mean curvature normal)**和**Gauss曲率法向(Gauss curvature normal)**：
...
类似地，对平均曲率法向和Gauss曲率法向进行积分可以得到离散曲面对应的曲率向量：
...
<div align=center>
<img src="https://i.imgur.com/RcD0lSq.png" width="70%">
</div>

首先考虑Gauss曲率，显然扩张后的平面和柱面上Gauss曲率为0，只有原来顶点对应的球面上存在非0的曲率。对于球面部分任一点的Gauss曲率为$K = \frac{1}{\varepsilon^2}$，在顶点$i$对应的球面上进行积分得到$K_i = (\Omega_i \varepsilon^2) \frac{1}{\varepsilon^2} =\Omega_i$，恰好等于顶点的angle defect。同时根据Gauss-Bonnet定理，Gauss曲率的积分为：
