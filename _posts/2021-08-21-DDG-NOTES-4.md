...
最后我们来介绍如何最小化共形能量。对于Dirichlet能量项，利用Laplace算子可以转换为：
...
不难发现此时优化问题等价于求解复数矩阵$C$的最小特征只对应的特征向量，可以使用**反幂法(inverse iteration)**来迭代求解。迭代格式如下：

1. 初始化向量$z_0$；
2. 求解线性方程：$C z_i = z_{i-1}$；
3. 平移到原点：$z_i = z_i - \bar{z}_i$；
4. 进行缩放：$z_i = z_i / \Vert z_i \Vert^2$；
5. 返回步骤2直至残差$C z_i - (z_i^H C z_i) z_i$收敛。
