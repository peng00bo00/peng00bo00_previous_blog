...
计算边界曲线的过程可参考如下`bestFitCurve()`函数。
...
$$
\mathbf{g} = \Lambda_\phi^{\dagger} \mathbf{h} = \phi_\mathbf{B}
$$
...
BFF算法的最后一步是根据参数化后的边界曲线$\tilde{\gamma}$来求解共形映射。为了保证计算得到的映射是全纯的，我们需要分别考虑实部和虚部。对于实部$a$只需要求解具有Dirichlet边界的Laplace方程：
