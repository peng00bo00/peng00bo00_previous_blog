...
从信息论的角度上看对任意随机事件$x$进行编码所需的最小字节数为$- \log P(x)$，因此最大后验也可以认为是选择最小编码长度的假设：似然项$-\log P(D \vert h)$表示给定假设$h$对数据集$D$进行编码所需的长度，$h$生成数据集$D$的概率越大所需字节数越小；而先验项$-\log P(h)$则表示假设$h$自身的复杂度，$h$越简单它出现的概率越高对它进行编码所需的字节数也越小。这说明最大后验估计会倾向于选择更为简单的假设，从而防止出现过拟合的问题。
