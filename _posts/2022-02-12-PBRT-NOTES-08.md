...
在很多情况下我们需要处理由参与介质构成几何模型的情况。此时光线与模型相交后不会改变前进路线，而是进入到介质中继续传播。为了处理这种情况，在PBRT中允许表面的材料指针为`nullptr`，这表示相交表面不会影响光线的传输；同样地，此时相交对象的BSDF指针`SurfaceInteraction::bsdf`也为`nullptr`，表示光线不会在表面发生散射。
...
在初始化时`BSSRDF`必须提供出射点`po`以及材料的折射率`eta`，而在求值时会调用`BSSRDF::S()`函数计算`pi`位置以`ei`角度入射后材质返回的BSSRDF函数值。
...
其中第一项$(1 - F_r(\cos \theta_o))$对应从出射点通过透射离开材料的比例；第二项$S_p(p_o, p_i)$表示光线从入射点到出射点进行传输的比例；第三项$S_\omega(\omega_i)$则是从入射位置进入材料的能量比例。对于高 albedo的材料，它们往往表现出更强的各向同性而且Fresnel项对最终的方向相关的结果影响更大，因此这一近似方法较为准确；而对于低albedo的材料这样的近似则不够准确。
...
和BRDF类似，我们同样可以使用一个表格来存储材料的BSSRDF，这在PBRT中使用`TabulatedBSSRDF`类来表示。
...
需要注意的是当材料确定时$S_r$实际上只是一个一维函数。更确切地说$S_r$是一个关于材料折射率$\eta$、散射各向异性$g$、反照率(albedo)$\rho$、衰减率$\sigma_t$以及距离$r$的函数。对于这样一个高维的函数直接进行离散化是不现实的，因此需要对这些参数进行简化。简化后的$S_r$使用固定的$\eta$和$g$，同时将物理距离$r$转换为无量纲的光学距离$r_\text{optical} = \sigma_t r$。这样$S_r$就化简为：
...
还需要注意的是`TabulatedBSSRDF::rho`表示的是单次散射的能量损失，而材料的albedo则是经过多次散射后能量损失。为了区分这两个概念我们把后者记为$\rho_\text{eff}$，它存储在`BSSRDFTable::rhoEff`中：
