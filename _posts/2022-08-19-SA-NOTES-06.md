...
平均曲率与曲面的面积有深刻的联系，这里我们用variational calculus来推导它们之间的关系。首先我们引入局部参数化$f_t(\mathbf{u}, \mathbf{v}): U 	\subseteq \mathbb{R}^2 \rightarrow \mathbb{R}^3$，此时可以定义三个导数$T_\mathbf{u} = \frac{\partial f_t}{\partial \mathbf{u}}$、$T_\mathbf{u} = \frac{\partial f_t}{\partial \mathbf{v}}$以及$w_t = \frac{\partial f_t}{\partial t}$。

<div align=center>
<img src="https://i.imgur.com/OqmYnWI.png" width="80%">
</div>

我们可以把$w_t$理解为一个向量场，它会引导整个曲面的变形。我们的目标是计算Gateaux导数$dA_{\mathcal{M}} (w_0)$
