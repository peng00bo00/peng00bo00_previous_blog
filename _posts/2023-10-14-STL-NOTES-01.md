...
`Function`在进行实例化时，具有相同签名`<Ret(Args...)>`的只会实例化出一份代码。因此只要函数对象具有相同的签名，我们就可以使用同一个`Function`类来进行处理；而对于具有相同签名但不同类型的函数对象，编译器会实例化出相应的`FuncImpl`类而无需重新实例化整个`Function`类。这种机制称为**类型擦除**，它允许在不同函数对象签名下实现通用性，同时保持类型安全和高效性。
