...
接下来我从零开始实现一个类似于标准库中`std::function`的函数容器`Function`。首先是模板定义：
...
这里`Function`模板有两个定义：第一个的定义是一个通用定义；而第二个的定义则是一个模板特化，用来接收`<Ret(Args...)>`形式的模板参数。这样做的意义在于只有当我们使用了`<Ret(Args...)>`形式的模板参数才会生成具体的代码，否则会并在编译期报错。
