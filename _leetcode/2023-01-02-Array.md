...
除此之外，也可以利用快慢指针来模拟字符串的退格过程。当快指针`fast`遇到非`#`字符串时将`fast`指向的元素赋给慢指针`slow`，并且将慢指针右移；否则将慢指针`slow`左移，遍历完成后取`slow`前面的字符即可。此时慢指针`slow`前面的元素就都是退格后保存下的字符，我们只需要比较`s`和`t`两个字符串经过退格后保存的元素是否一一对应相等即可。
